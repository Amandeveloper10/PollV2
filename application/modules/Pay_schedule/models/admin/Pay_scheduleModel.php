 ...
 */
class Pay_scheduleModel extends CI_Model {
    ...
    public function load_all_data() {
        $this->db->select('t1.*');
        $this->db->from(tablename('state') . ' as t1');
  ...
  /**
     ...
     */
     ...
     public function modify($id) {  
      //echo "<pre>"; print_r($_POST); die;
        ...
        $processing_month = $this->input->post('processing_month');
        $day_of_processing = $this->input->post('period_day');
        ...
        if (!empty($editid)) {
            ...
            $this->db->where('id', $editid)->update(tablename('pay_schdule'), $data);
            $this->session->set_flashdata('successmessage', 'pay schdule modified successfully');
            redirect(base_url('page/79/1'));
        } else {
            ...
            $this->db->insert(tablename('pay_schdule'), $data);
            
             $this->session->set_flashdata('successmessage', 'pay schdule added successfully');
            redirect(base_url('page/79/1'));
        }
    }
    ...
    public function load_single_data($id = "") {
        $this->db->select('*');
        $this->db->from(tablename('pay_schdule'));
        $this->db->where('id', $id);
        $query = $this->db->get();
        $result = $query->row();
