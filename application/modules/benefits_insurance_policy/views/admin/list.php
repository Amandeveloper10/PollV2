<div class="ks-page-header">
    <section class="ks-title">
        <h3>Benefits / Insurance Policy</h3>
                ...
                <?php
                    ...
                    if (!empty($all_data)) {
                      foreach ($all_data as $data) {
                        $employee = $this->BenefitInsModel->get_row_data('employee',array('id'=>$data->employee_id));
                    ?>
                  <tr>
...
<script type="text/javascript">
    function openForm(id){
         $.post("<?php echo base_url('admin_get_edit_form_benefits_insurance_policy'); ?>", {id: id}, function(result){
            //console.log(result);
    ...
    function change_status(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
                var url = "<?php echo base_url('admin_status_benefits_insurance_policy'); ?>/" + id;
            ...
            $("#notification_body").html("Do you want to change status of this Benefits / Insurance Policy?");
        }).modal("show");
    }
     ...
     function delete_this(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
               var url = "<?php echo base_url('admin_delete_benefits_insurance_policy'); ?>/" + id;
                $("#pjax-container").load(url,function(response,status){       
                    $('#ks-datatable').bootstrapTable({
                        icons: {
                            refresh: 'la la-refresh icon-refresh',
            ...
            $("#notification_heading").html("Confirmation");
            $("#notification_body").html("Do you want to Delete this Benefits / Insurance Policy?");
        }).modal("show");
    }
</script>