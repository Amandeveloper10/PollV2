<div class="ks-page-header">
    <section class="ks-title">
        <h3> <span>Settings > Employee Master ></span> Department</h3>
                ...
                <?php
                    ...
                    if (!empty($all_data)) {
                      foreach ($all_data as $data) {
						  $deparment = $this->DeptModel->get_row_data('master_department', array('id' => $data->under));
                    ?>
                  <tr>
                    <td> <a class="pjaxCls" href="javascript:void(0)" onclick="openForm(<?php echo $data->id; ?>);"><?php echo $data->department_name; ?></a></td>
                    <td><?php if($data->under == '0'){ echo 'Primary'; } else { echo $deparment->department_name;} ?></td>
...
function btnCloseForm(){
    ...
    function openForm(id){
         $.post("<?php echo base_url('admin_get_edit_form_department'); ?>", {id: id}, function(result){
            //console.log(result);
    ...
    function change_status(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
                var url = "<?php echo base_url('admin_status_department'); ?>/" + id;
            ...
            $("#notification_body").html("Do you want to change status of this department?");
        }).modal("show");
    }
     ...
     function delete_this(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
               var url = "<?php echo base_url('admin_delete_department'); ?>/" + id;
                $("#pjax-container").load(url,function(response,status){                     
                    notification(status);
                });
            });
            ...
            $("#notification_heading").html("Confirmation");
            $("#notification_body").html("Do you want to Delete this department?");
        }).modal("show");
    }
</script>
