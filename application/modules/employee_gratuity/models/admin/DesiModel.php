...
class DesiModel extends CI_Model {
	...
	public function load_all_employee_salary_details($sal_id) {
		$component_id = array('3', '5');
        $this->db->select('t1.*');
        $this->db->from(tablename('employee_salary_details') . ' as t1');
		$this->db->where('t1.employee_salary_id',$sal_id);
		$this->db->where_in('t1.component_id',$component_id);
	...
	public function load_all_employee_salary($emp_id) {
        $this->db->select('t1.*');
