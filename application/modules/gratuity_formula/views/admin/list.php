<div class="ks-page-header">
    <section class="ks-title">
        <h3><span>Rules > </span> Gratuity Formula <small>(for all)</small></h3>
        ...
        <div class="content-wrapper">            
            ...
            <table id="payrolltable" class="table table-list">
                ...
                <?php
                    ...
                    ?>
                  <tr>
                    <td><a class="" href="javascript:void(0)" onclick="openForm(<?php echo $data->id; ?>);"><?php echo @$data->type; ?></a></td>
                    <td> <?php //echo @$data->operator; ?>  Gratuity = <?php echo @$data->no_of_year; ?> <?php echo @$data->operator; ?> (<?php echo @$data->based_on; ?><?php echo @$data->operator_1; ?><?php echo @$data->based_on_1; ?>) <?php echo @$data->operator; ?> <?php echo @$data->fixed_value; ?></td>
...
// datatable end 
    ...
    function openForm(id){
         $.post("<?php echo base_url('admin_get_edit_form_gratuity_formula'); ?>", {id: id}, function(result){
            //console.log(result);
    ...
    function change_status(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
                var url = "<?php echo base_url('admin_status_gratuity_formula'); ?>/" + id;
     ...
     function delete_this(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
               var url = "<?php echo base_url('admin_delete_gratuity_formula'); ?>/" + id;
                $("#pjax-container").load(url,function(response,status){                           
                    notification(status);
                });
            });
            ...
            $("#notification_heading").html("Confirmation");
            $("#notification_body").html("Do you want to Delete this Gratuity Formula?");
        }).modal("show");
    }

    function getOtFormulaGratuity() {

      var based_on = $("#based_on option:selected").text();
      var based_on_1 = $("#based_on_1 option:selected").text();
      var operator = $("#operator option:selected").text();

      //var day_id = $("#day").val();
      var year_id = $("#no_of_year").val();
     ...
     $('#add_id').html('+');
      $('#operator_id').html(operator);
       $('#operator_id_1').html(operator);
      $('#year_id').html(year_id);
      $('#based_on_id').html(based_on);
      $('#based_on_id_1').html(based_on_1);
    }
