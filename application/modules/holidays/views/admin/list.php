...
<div class="ks-page-header">
    <section class="ks-title">
        <h3><span>Core HR > </span> Holidays</h3>
            ...
            <table id="payrolltable" class="table table-list">
                ...
                <?php
                  ...
                  <tr>
                    ...
                    <td> <a class="" href="javascript:void(0)" onclick="openForm(<?php echo $data->id; ?>);"><?php echo $data->holiday_name; ?></a></td>
					<td><?php $location_id = $this->HolidaysModel->get_row_data('master_holidays_location',array('master_holiday_id'=>$data->id));
					          $company_name = $this->HolidaysModel->get_row_data('setting_organization',array('id'=>$location_id->organization_id));
							  $count_location = $this->HolidaysModel->get_result_data('master_holidays_location',array('master_holiday_id'=>$data->id));
							  $plus = '';
							  if (count($count_location) > '1'){
								  $pluss = count($count_location) - 1;
								  $plus = '+'.$pluss;
							  }
					echo $company_name->company_name.' '.$plus;
					//=$data->company_name?></td>
                    <td><?php echo ($data->holiday_start_date == $data->holiday_end_date) ? date($SysConfigauthenticaton->date_format,strtotime($data->holiday_start_date)) : date($SysConfigauthenticaton->date_format,strtotime($data->holiday_start_date)) .' to '. date($SysConfigauthenticaton->date_format,strtotime($data->holiday_end_date)); ?></td>
                    <td><?php echo $data->holiday_type; ?></td>
                    <td><?php echo $data->holiday_off_type; ?></td>
...
// datatable end 
    ...
    function openForm(id){
         $.post("<?php echo base_url('admin_get_edit_form_holidays'); ?>", {id: id}, function(result){
            //console.log(result);
    ...
    function change_status(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
                var url = "<?php echo base_url('admin_status_holidays'); ?>/" + id;
            ...
            $("#notification_body").html("Do you want to change status of this Holidays?");
        }).modal("show");
    }
     ...
     function delete_this(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
               var url = "<?php echo base_url('admin_delete_holidays'); ?>/" + id;
                $("#pjax-container").load(url,function(response,status){
                    notification(status);
                });
            });
            ...
            $("#notification_heading").html("Confirmation");
            $("#notification_body").html("Do you want to Delete this Holidays?");
        }).modal("show");
    }
</script>