<div class="ks-page-header">
    <section class="ks-title">
        <h3><span>HR > </span> Set Formula</h3>
...
$.post("<?php echo base_url('div_benefit_colon'); ?>", {id: div_id,value: myList}, function(result){   
    ...
    function openForm(id=""){
         $.post("<?php echo base_url('admin_get_edit_form_set_formula'); ?>", {id: id}, function(result){
            //console.log(result);
  ...
  if(component!='' && base_on!='' &&oparetor!='' && amount!=''){
     ...
     var basicnow='';
      ...
      var basicnow= parseFloat((parseFloat(ctc) - parseFloat(amount/100)));
         ...
         $('#basic_amount').html(basicnow);
         $('#basic_amount_monthly').html(basicnow/12);
            }
            else{
      ...
      var calculate_amount= parseFloat((parseFloat(type) - parseFloat(amount/100)));
         ...
         $('#calculate_amount'+id).html(calculate_amount);    
          $('#monthly_calculate_amount'+id).html(calculate_amount/12);      
            }
      ...
      if(parseFloat(totalAmoutctc)>parseFloat(ctc)){
        var balancing_amount  = parseFloat(ctc) - parseFloat(totalctc);
           ...
           $('#adjustment_amount').val(adjustment_amount);
            $('#total_adjustment_amount').text(adjustment_amount);
            $('#monthly_adjustment_amount').text(adjustment_amount/12);
...
$('#monthly_ctc_amount').text(totalAmoutctc/12);
...
var totalctc = $('#total_ctc_amount').text();
     ...
     function salarycalculation_new(value,id) {
        ...
        // alert(id);
         ...
         $('#calculate_amount'+id).html(basicnow);
          $('#monthly_calculate_amount'+id).html(basicnow/12);
        $('.total_amount').each(function() {
        // console.log($(this).text());
        total += parseFloat($(this).text());
        });
