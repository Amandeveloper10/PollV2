<div class="ks-page-header">
    <section class="ks-title">
        <h3><span>Rules > </span> Late Rules</h3>
                ...
                <?php
                    ...
                    if (!empty($all_data)) {
                      foreach ($all_data as $data) {
                        $rulename = '';
						$nos = '';
						//echo '<pre>'; print_r($data->leave_type);
						if(!empty($data->leave_type)){
							$nos = $data->nos;
                        $leave_type = explode(',',$data->leave_type);
                        foreach ($leave_type as $key => $value) {
                          $all_leaverule = $this->LateRulesModel->get_row_data('master_leave_rules',array('id'=>$value));
						  //echo $all_leaverule->rule_name; echo '<hr>';
                          $rulename .= $all_leaverule->rule_name.',';
						  //echo $rulename; echo '<hr>';
                        }
						}
                       //echo $rulename;
                    ?>
                  <tr>
                    <td> <a class="" href="javascript:void(0)" onclick="openForm(<?php echo $data->id; ?>);"><?php echo $data->rule_name; ?></a></td>
                     <td><?php echo $data->occurrence_type; ?></td>
                    <td><?php echo $data->no_of_occu_allowed; ?></td>
                    <td><?php echo $data->type; ?></td>
                    <td><?php echo $data->include; ?></td>
                    <td><?php echo rtrim($rulename, ','); ?></td>
                    <td><?php echo $nos; ?></td>
...
// datatable end 
    ...
    function openForm(id){
         $.post("<?php echo base_url('admin_get_edit_form_late_rules'); ?>", {id: id}, function(result){
            //console.log(result);
    ...
    function change_status(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
                var url = "<?php echo base_url('admin_status_late_rules'); ?>/" + id;
            ...
            $("#notification_body").html("Do you want to change status of this Late Rules?");
        }).modal("show");
    }
     ...
     function delete_this(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
               var url = "<?php echo base_url('admin_delete_late_rules'); ?>/" + id;
                $("#pjax-container").load(url,function(response,status){       
                    notification(status);
                });
            });
            ...
            $("#notification_heading").html("Confirmation");
            $("#notification_body").html("Do you want to Delete this Late Rules?");
        }).modal("show");
    }
    ...
    function getDedLeave() {
      ...
      if(checkBoxes.prop("checked") == true){
		  ...
		  $("#leave_type"). attr("required", "true");
		  $("#nos"). attr("required", "true");
      }
      else if(checkBoxes.prop("checked") == false){
          $(".deduDiv").hide();
          $("#deduction_apply1").val('N');
		  $("#leave_type"). attr("required", "false");
		  $("#nos"). attr("required", "false");
      }
    }
</script>