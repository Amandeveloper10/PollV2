<div class="ks-page-header">
    <section class="ks-title">
        <h3> <span>Core HR ></span> Notice</h3>
        ...
        <div class="content-wrapper">            
            ...
            <table id="payrolltable" class="table table-list">
                ...
                <?php
                    ...
                    ?>
                  <tr>
				     <td> <a class="" href="javascript:void(0)" onclick="openForm(<?php echo $data->id; ?>);"><?php echo $data->subject; ?></a></td>
                      <td><!--<?php echo @$data->first_name . ' ' . @$data->middle_name . ' ' . @$data->last_name; ?><?php if(!empty($data->employee_no)) { echo '('.$data->employee_no.')'; } ?>-->
					  <?php if(!empty($data->employee_id)){
                      $employeeID = explode(',', $data->employee_id);
                      echo count($employeeID);
                      /*foreach($employeeID as $key => $value){
                        
                      $all_emp = $this->NoticeModel->get_row_data('employee',array('is_active'=>'Y','id'=>$value,'delete_flag'=>'N'));
                      $empName = $all_emp->first_name. ' ' . @$all_emp->middle_name . ' ' . @$all_emp->last_name;
...
function btnCloseForm(){
    ...
    function openForm(id){
         $.post("<?php echo base_url('admin_get_edit_form_notice'); ?>", {id: id}, function(result){
            //console.log(result);
    ...
    function change_status(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
                var url = "<?php echo base_url('admin_status_notice'); ?>/" + id;
            ...
            $("#notification_body").html("Do you want to change status of this notice?");
        }).modal("show");
    }
     ...
     function delete_this(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
               var url = "<?php echo base_url('admin_delete_notice'); ?>/" + id;
                $("#pjax-container").load(url,function(response,status){
                    notification(status);
                });
            });
            ...
            $("#notification_heading").html("Confirmation");
            $("#notification_body").html("Do you want to Delete this notice?");
        }).modal("show");
    }
</script>
