<div class="ks-page-header">
    <section class="ks-title">
        <h3><span>Rules > </span> Overtime Formula <small>(Applicable for all)</small></h3>
            ...
            <table id="payrolltable" class="table table-list">
                ...
                <?php
                    ...
                    if (!empty($all_data)) {
                      ...
                      $component = $this->OvertimeRulesModel->get_row_data('master_salary_component',array('id'=>$data->component));
                    ?>
                  <tr>
                    <td> <a class="" href="javascript:void(0)" onclick="openForm(<?php echo $data->id; ?>);"><?php echo $data->overtime_name; ?></a></td>
                    <td><?php echo $data->overtime_type; ?></td>
                    <td><?php echo @$component->component; ?></td>
                    <td><?php echo $data->day; ?></td>
                    <td><?php echo $data->hour; ?></td>
                    <td><?php echo $data->multiply_by; ?></td>
...
// datatable end 
    ...
    function openForm(id){
         $.post("<?php echo base_url('admin_get_edit_form_overtime_rules'); ?>", {id: id}, function(result){
            //console.log(result);
    ...
    function change_status(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
                var url = "<?php echo base_url('admin_status_overtime_rules'); ?>/" + id;
     ...
     function delete_this(id) {
        $("#myModal").on('shown.bs.modal', function() {
            $("#modal_confirm").click(function() {
               var url = "<?php echo base_url('admin_delete_overtime_rules'); ?>/" + id;
                $("#pjax-container").load(url,function(response,status){                           
                    notification(status);
                });
            });
    ...
    function getOtFormula() {
      var component_val = $("#component option:selected").text();
      //console.log(component_val);
      var day_val = $("#day").val();
      var day_val = $("#day").val();
      var hour_val = $("#hour").val();
      var multiply_by_val = $("#multiply_by").val();

      $('#component_type').html(component_val);
      $('#day_id').html(day_val);
      $('#hour_id').html(hour_val);
      $('#multiply_by_id').html(multiply_by_val);
    }
