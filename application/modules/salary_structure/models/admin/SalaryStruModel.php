...
class SalaryStruModel extends CI_Model {
	...
	public function count_emp_salary($strid) {
        $this->db->select('t1.*');
        $this->db->from(tablename('employee_salary') . ' as t1');
        $this->db->where('t1.salary_structure_id', $strid);
    ...
    public function modify($id) {
       ...
       // echo "<pre>"; print_r($_POST); die;
        ...
        $ctc_amount = $this->input->post('ctc_amount');
        ...
        $not_part_ctc_component_id = $this->input->post('not_part_ctc_component_id');
...
//echo '<pre>'; print_r($_POST); die;
        ...
        if (!empty($id)) {
            $data = array(
                ...
                'dept_id' => $dept_id,
                'modified_date' => $date,
                'not_part_ctc_component_id' => $not_part_ctc_component_id
            );
...
//echo '<pre>';print_r($component_deduction);
//echo '<pre>';print_r($dedfixed_amount);
//exit;
...
//echo '<pre>';print_r($component_deduction);exit;
        ...
        $amountbenefit = explode(',',$this->input->post('amountbenefit')[0]);
        $benefit_base_on = explode(',',$this->input->post('benefit_base_on')[0]);
        $fixed_benefit_amount = explode(',',$this->input->post('fixed_benefit_amount')[0]);
        ...
        if (!empty($component_benefit)) {
                foreach ($component_benefit as $key => $value) {
                    $databenefit = array(
                        ...
                        'salary_type' => 'Annual',
                        'amount' => $amountbenefit[$key]
                    );
                   // echo$amount[$key];
                    if(empty($amountbenefit[$key]))
                    {

                       $databenefit['fixed_amount'] = (float) $fixed_benefit_amount[0];
...
if(!empty($_POST['value']))
        {
         ...
         //print_r($value);exit;
     $sql = "select * from HR_master_salary_component where is_active='Y' and delete_flag='N' and mode='Monthly' and type='Earning' and id !='3' and id !='20' and id not in ($value) ";
        }
        else{
            $sql = "select * from HR_master_salary_component where is_active='Y' and mode='Monthly' and id !='20' and type='Earning' and delete_flag='N' ";
        }
...
if(!empty($_POST['value']))
        {
         $value=     implode( "," , $_POST['value']);
         //print_r($value); die;
     $sql = "select * from HR_master_salary_component where is_active='Y' and delete_flag='N' and type='Deduction' and mode='Monthly' and id not in ($value) ";
        }
        else{
            $sql = "select * from HR_master_salary_component where is_active='Y' and mode='Monthly' and type='Deduction' and delete_flag='N' ";
        }
		//echo $sql; die;
