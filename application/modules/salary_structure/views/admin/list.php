<div class="ks-page-header">
    <section class="ks-title">
		<h3><span>Settings > HR Master ></span> Salary Structure</h3>
            ...
            <table id="payrolltable" class="table table-list">
                ...
                <?php
                    ...
                    if (!empty($all_data)) {
                      ...
                      //$emp_salary_str = $this->SalaryStruModel->get_row_data('employee_salary',array('salary_structure_id'=>$data->id));
					  $count_emp = $this->SalaryStruModel->count_emp_salary($data->id);
					  //echo '<pre>'; print_r($count_emp); //die;
					  $count = 0;
					  if(!empty($count_emp)){
						  $count = count($count_emp);
					  }else{ 
						  $count = 0;
					  }
                  ...
                  <tr>
                    ...
                    <td><?php echo @$dept->department_name; ?></td>
					 <td><?php if(!empty($count_emp)) { echo count($count_emp); } else { echo '0'; } ?></td>
...
var div_id_type = (parseInt(benefit_div_type) + 1 );
             ...
             // alert(div_id);
              ...
              $('#component_type_new'+div_id).html(type[0]+','+type[1]);
                  ...
                  if(type[4].trim()=='Yes'){
                     ...
                     $('#calculate_amount'+div_id).removeClass("pfNo");
                  }else{
                     $('#calculate_amount'+div_id).removeClass("pfYes");
                     $('#calculate_amount'+div_id).addClass("pfNo");
                  }
                   if(type[5].trim()=='Yes'){
  ...
  if(component!='' && base_on!='' &&oparetor!='' && amount!=''){
     ...
     var basicnow='';
      ...
      var basicnow= parseFloat((parseFloat(ctc) - parseFloat(amount/100)));
         ...
         // var basicnow= parseFloat(((amount/100)*ctc));
         $('#deduction_amount').html(Math.round(basicnow));
		 $('#deduction_amount_monthly').html(Math.round(basicnow/12));
            }
            else{
      ...
      var calculate_amount= parseFloat((parseFloat(type) - parseFloat(amount/100)));
         ...
         $('#calculate_deductionamount'+id).html(Math.round(calculate_amount));   
		$('#calculate_deductionamountmonthly'+id).html(Math.round(calculate_amount/12));		 
...
var totalctc = $('#total_ctc_amount').text();
     ...
     function salarybenefitcalculation(value,i) {
      //alert(value);
    var benefit_div = $("#benefit_div_type").val();

    var ids = (parseInt(benefit_div) + 1 );
    if(ids == 'undefined'){
      var id = 1;
    }else{
      var id = ids;
  ...
  if(component!='' && base_on!='' &&oparetor!='' && amount!=''){
     ...
     var basicnow='';
      ...
      var basicnow= parseFloat((parseFloat(ctc) - parseFloat(amount/100)));
         ...
         // var basicnow= parseFloat(((amount/100)*ctc));
         $('#benefit_amount_monthly'+id).html(Math.round(basicnow/12));
         $('#benefit_amount_annual'+id).html(Math.round(basicnow));
            }
            else{
      ...
      var calculate_amount= parseFloat((parseFloat(type) - parseFloat(amount/100)));
         ...
         $('#calculate_benefitamount'+id).html(Math.round(calculate_amount));    
...
var totalctc = $('#total_ctc_amount').text();
    ...
    function salarydeductioncalculation_new(value,id) {
         ...
         $('#calculate_deductionamount'+id).html(Math.round(basicnow));
		 $('#calculate_deductionamountmonthly'+id).html(Math.round(basicnow/12));
        var totaldeduction = 0;
                    $('.total_deduction').each(function() {
   ...
   if ($('#esi_include').prop('checked')) {
     ...
     function salarybenefitcalculation_new(value,i) {
         if(value==''){
         value   =0; 
        }
         var benefit_div = $("#benefit_div_type").val();
	...
	function get_alert(){
		...
		$("#myModal").on('shown.bs.modal', function() {
            ...
            $("#notification_body").html("Can not delete this structure until it is '0'.");
        }).modal("show");
	}
</script>